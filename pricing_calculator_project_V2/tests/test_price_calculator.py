...
# --------------------------------------------------------------------------- #
def test_coeficiente_padrao():
    assert DEFAULT_COEFFICIENT_36M == 0.065


def test_precos_a_partir_do_custo_exemplo_briefing():
    """Custo R$ 3.000 com coeficiente 6,5% deve bater com o exemplo do briefing."""
    p = calculate_prices_from_cost(3000)
    assert p == {
        "preco_12m": 235.95,
        "preco_24m": 214.50,
        "preco_36m": 195.00,
...
def test_coeficiente_customizado():
    p = calculate_prices_from_cost(2000, coefficient_36m=0.05)
    assert p["preco_36m"] == 100.00


@pytest.mark.parametrize("valor_invalido", [0, -1, -3000, None, ""])
def test_custo_invalido_levanta_erro(valor_invalido):
    with pytest.raises(ValueError):
        calculate_prices_from_cost(valor_invalido)
...
def test_custo_a_partir_do_preco_36m():
    assert calculate_cost_from_36m_price(195.00) == 3000.00


def test_custo_reverso_coeficiente_customizado():
    assert calculate_cost_from_36m_price(100.00, coefficient_36m=0.05) == 2000.00


def test_ida_e_volta_consistente():
    """custo -> 36m -> custo deve retornar ao valor original."""
    p = calculate_prices_from_cost(7500)
    assert calculate_cost_from_36m_price(p["preco_36m"]) == 7500.00


@pytest.mark.parametrize("preco_invalido", [0, -10, None, ""])
def test_preco_36m_invalido_levanta_erro(preco_invalido):
    with pytest.raises(ValueError):
        calculate_cost_from_36m_price(preco_invalido)
...
def test_prazos_a_partir_de_36m():
    p = calculate_prices_from_36m(195.00)
    assert p == {
        "preco_12m": 235.95,
        "preco_24m": 214.50,
        "preco_36m": 195.00,
...
def test_from_cost_e_from_36m_coincidem():
    custo = calculate_prices_from_cost(3000)

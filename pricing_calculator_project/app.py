...
from src.auth import require_login, render_logout, is_admin_user
...
def get_default_coef_pct() -> float:
    ...
    try:
        ...
        elif "credentials" in st.secrets and "default_coefficient_36m" in st.secrets["credentials"]:
            # Compatibilidade: formatos antigos do guia podiam aninhar esta
            # chave dentro de [credentials] por engano (comportamento do TOML).
            val = float(st.secrets["credentials"]["default_coefficient_36m"])
    except Exception:
        val = None
    if val is None:
...
IS_ADMIN = is_admin_user(st.session_state.get("auth_user", ""))
...
if IS_ADMIN:
    ...
    tab_precificacao, tab_reverso, tab_relatorio, tab_comprovacao = _tabs
else:
    _tabs = st.tabs(["  Precificação  ", "  Relatório do Vendedor  "])
    tab_precificacao, tab_relatorio = _tabs
    tab_reverso = None
    tab_comprovacao = None
    ...
    if add_modo.startswith("Por custo"):
        ...
        with cc2:
            if IS_ADMIN:
                ...
                add_coef = get_default_coef_pct()
                st.text_input("Coeficiente 36 meses", value="Aplicado automaticamente", disabled=True)
    else:
...
# =========================================================================== #
# ABA 2 — CÁLCULO REVERSO (somente admin)                                     #
# =========================================================================== #
if IS_ADMIN:
    with tab_reverso:
...
# =========================================================================== #
# ABA 4 — COMPROVAÇÃO DE CUSTO (memória de cálculo + evidência, só admin)     #
# =========================================================================== #
if IS_ADMIN:

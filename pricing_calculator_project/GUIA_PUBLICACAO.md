...
## Parte 3 — Configurar usuários, coeficiente e acesso total (Secrets)
...
2. Cole e ajuste — pode adicionar **quantos usuários quiser**, um por linha:
   ...
   admin_users = ["seu-usuario"]

   [credentials]
   seu-usuario = "uma-senha-forte"
...
> **Atenção à ordem das linhas.** Em TOML, tudo que vem **depois** de um
> cabeçalho `[tabela]` é lido como pertencente a essa tabela. Por isso
> `default_coefficient_36m` e `admin_users` ficam **antes** de
> `[credentials]` — se ficarem depois, são lidos como se fossem parte de
> `[credentials]` e deixam de funcionar (o coeficiente volta a usar um valor
> genérico, e a restrição de acesso não é aplicada).

### Quem vê o quê

`admin_users` define quem tem **acesso total**: vê e edita o coeficiente,
acessa a aba **Cálculo Reverso** e a aba **Comprovação de Custo** (memória
de cálculo). Quem **não** estiver nessa lista só acessa **Precificação**
(sem ver o coeficiente — ele é aplicado automaticamente, nos bastidores) e
**Relatório do Vendedor**.

> Se `admin_users` não for definida, **todos os usuários logados têm acesso
> total** (comportamento anterior, preservado para compatibilidade).

### Para adicionar um novo usuário depois

Volte em **Settings → Secrets**, acrescente uma linha dentro de
`[credentials]` (ex.: `vendedor3 = "senha-dele"`) e, se quiser dar acesso
total a essa pessoa também, inclua o nome dela em `admin_users`. Clique em
**Save**. Não precisa tocar no código.

### Para remover o acesso de alguém

Apague a linha correspondente em **Secrets** (em `[credentials]` e, se for
o caso, em `admin_users`) e clique em **Save**. A pessoa não conseguirá
mais entrar (ou perde o acesso total, se só foi removida de `admin_users`).
...
- **Coeficiente aparece errado, ou todo mundo continua vendo o coeficiente
  mesmo com `admin_users` configurada**: confira a ordem das linhas em
  Settings → Secrets — `default_coefficient_36m` e `admin_users` precisam
  vir **antes** de `[credentials]`, nunca depois (ver Parte 3).
